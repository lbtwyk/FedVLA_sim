...
class RobotEpisodeDataset(Dataset):
    ...
    def _load_data(self):
        ...
        Handles both flat directory structures (timestamped episodes) and numbered episodes.
        Stores image paths for later loading.
        """
        structure_type = "flat" if self.flat_structure else "numbered"
        ...
        if self.flat_structure:
            # Flat structure: episodes are directly in the base directory with timestamped names
            loaded_episodes_count, total_timesteps, skipped_timesteps = self._load_flat_structure(loaded_episodes_count, total_timesteps, skipped_timesteps)
        else:
            # Numbered structure: episodes named episode1, episode2, etc.
            loaded_episodes_count, total_timesteps, skipped_timesteps = self._load_numbered_structure(loaded_episodes_count, total_timesteps, skipped_timesteps)
    ...
    def _load_flat_structure(self, loaded_episodes_count, total_timesteps, skipped_timesteps):
        """
        Loads data from a flat directory structure (timestamped episodes directly in base_dir).
        """
        try:
            ...
            return loaded_episodes_count, total_timesteps, skipped_timesteps
    ...
    def _load_numbered_structure(self, loaded_episodes_count, total_timesteps, skipped_timesteps):
        """
        Loads data from numbered episode structure (episode1, episode2, etc.).
        """
    ...
    def _process_episode_dir(self, episode_dir, episode_index):
        ...
        """
        # Try both state.json and states.json for compatibility
        state_file_path = os.path.join(episode_dir, 'states.json')
        if not os.path.isfile(state_file_path):
            state_file_path = os.path.join(episode_dir, 'state.json')
            if not os.path.isfile(state_file_path):
                logging.warning(f"Neither states.json nor state.json found in {episode_dir}, skipping this episode.")
                return None
        ...
        try:
            ...
            if not isinstance(episode_data, list):
                logging.warning(f"JSON file in {episode_dir} is not a list, skipping episode data.")
                return None
            ...
            for timestep_data in episode_data:
                ...
                # Image path is relative to episode_dir
