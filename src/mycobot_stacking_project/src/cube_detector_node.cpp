...
void CubeDetectorNode::pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  ...
  for (const auto& indices : cluster_indices) {
    ...
    for (const auto& idx : indices.indices) {
      cluster->push_back((*cloud_objects)[idx]);
    }
    ...
    bool is_cube_sized = 
      std::abs(width - target_cube_size_) < size_tolerance_ &&
      std::abs(height - target_cube_size_) < size_tolerance_ &&
      std::abs(depth - target_cube_size_) < size_tolerance_;

    if (!is_cube_sized) {
      RCLCPP_DEBUG(this->get_logger(), "Cluster dimensions (%.3f, %.3f, %.3f) don't match cube size", 
                  width, height, depth);
      continue;
    }
    ...
    bool is_yellow = 
      r_avg >= yellow_color_range_.r_min && r_avg <= yellow_color_range_.r_max &&
      g_avg >= yellow_color_range_.g_min && g_avg <= yellow_color_range_.g_max &&
      b_avg >= yellow_color_range_.b_min && b_avg <= yellow_color_range_.b_max;

    bool is_orange = 
      r_avg >= orange_color_range_.r_min && r_avg <= orange_color_range_.r_max &&
      g_avg >= orange_color_range_.g_min && g_avg <= orange_color_range_.g_max &&
      b_avg >= orange_color_range_.b_min && b_avg <= orange_color_range_.b_max;
    ...
    if (is_yellow) {
      ...
      RCLCPP_DEBUG(this->get_logger(), "Cluster color doesn't match yellow or orange");
      continue;
    }
