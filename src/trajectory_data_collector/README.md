...
The trajectory data collector provides a system for recording robot demonstrations. In this project, these demonstrations are typically generated by the **MoveIt2-driven `stacking_manager_node`** (from the `mycobot_stacking_project` package) autonomously performing the cube stacking task.

- **State Logger Node**: Records synchronized robot states and camera images from these MoveIt2-driven demonstrations.
...
Ble robot states and camera images.

**Note on Overall Data Collection:** While this `state_logger_node` is responsible for the low-level recording of data for a single episode (e.g., a single MoveIt2-driven stacking attempt), the primary method for collecting multiple episodes for training is by using the `collect_multiple_episodes.sh` script located in the root of the `ros2_ws` workspace. That script automates the launching of `collect_data.launch.py` (which utilizes this `state_logger_node` to record the MoveIt2 demonstrations) for numerous episodes, handles failure detection, and manages the overall data collection campaign.
...
The state logger automatically integrates with the myCobot stacking manager (`stacking_manager_node`). When data collection is active (typically via `collect_data.launch.py`), the `stacking_manager_node` uses MoveIt2 to perform the stacking task, and this `state_logger_node` records the trajectory.

```python
# In stacking manager node (which uses MoveIt2 for task execution)
