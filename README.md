...
1. **Motion Planning**: MoveIt 2-based cube stacking with the `stacking_manager_node`
2. **Data Collection**: Automated recording of expert demonstrations via `collect_multiple_episodes.sh`
3. **Model Training**: Neural network training for visuomotor control (centralized or federated)
4. **Real-time Inference**: Trained diffusion policies control the robot via `run_realtime_inference.sh`

## Demo

🎥 **[Watch the Inference Demo](https://liveuclac-my.sharepoint.com/:v:/g/personal/zcabaax_ucl_ac_uk/EYtNW_m0uf9CknEi12tfdVcBMjQBGibRLapVYdBSpMbGwA?nav=eyJyZWZlcnJhbEluZm8iOnsicmVmZXJyYWxBcHAiOiJPbmVEcml2ZUZvckJ1c2luZXNzIiwicmVmZXJyYWxBcHBQbGF0Zm9ybSI6IldlYiIsInJlZmVycmFsTW9kZSI6InZpZXciLCJyZWZlcnJhbFZpZXciOiJNeUZpbGVzTGlua0NvcHkifX0&e=HV1eUH)**

See the trained diffusion policy model performing real-time visuomotor cube stacking in simulation.

![Demo Snapshot](Final_demo_cover.jpg)

### Key Features

- **Gazebo Garden**: Enhanced physics and friction simulation
- **myCobot 280**: 6-DOF arm with gripper control
- **Vision System**: 424x240 camera for visuomotor learning
- **Data Pipeline**: Synchronized joint states, gripper positions, and images
- **PyTorch Training**: Diffusion policy with data augmentation
...
├── DP/                             # Centralized diffusion policy training system
│   ├── model.py                    # Neural network architecture
│   ├── train.py                    # Training script with enhanced diffusion sampling
│   ├── dataset.py                  # Data loading and preprocessing
│   ├── augmentation.py             # State augmentation for robust training
...
│   ├── inference_gazebo.py         # Gazebo integration inference (legacy)
│   ├── inference_modified.py       # Modified inference variants (legacy)
│   ├── requirements.txt            # Python dependencies
│   └── checkpoints/                # Trained model files
├── flwr_DP/                        # Federated learning training system
...
│   ├── flwr_client.py              # Flower federated learning client
│   ├── flwr_server.py              # Flower federated learning server
...
### Training and Models
- **[Centralized Diffusion Policy Training](DP/README.md)** - Enhanced model training with augmentation
- **[Federated Learning System](flwr_DP/README.md)** - Distributed training with Flower framework
...
- **Training Options**: Centralized training (DP/) or federated learning (flwr_DP/)
- **Data Augmentation**: State-space augmentation for improved robustness
